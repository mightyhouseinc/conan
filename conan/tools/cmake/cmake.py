...
class CMake(object):
    ...
    def install(self, build_type=None, component=None, cli_args=None, stdout=None, stderr=None):
        ...
        if do_strip := self._conanfile.conf.get(
            "tools.cmake:install_strip", check_type=bool
        ):
    ...
    def ctest(self, cli_args=None, env="", stdout=None, stderr=None):
        ...
        if verbosity := self._conanfile.conf.get(
    ...
    def _cmake_log_levels_args(self):
        ...
        if log_level == "quiet":
            log_level = "error"
        return [f"--loglevel={log_level.upper()}"] if log_level is not None else []
